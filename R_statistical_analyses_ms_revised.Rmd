...
library(ggpubr); packageVersion("vegan")
...
source('Y:/R_analyses/Takahe_Final/ggrarefaction_curves.R')
library(vegan)

rcurves <- ggrare(ps2, step = 500, se = FALSE) 
rcurves_intcpt <- rcurves + geom_vline(xintercept=8000, color = "blue")
ggsave("rarefaction_curves_16S_takahe.png", rcurves_intcpt, height = 10, width = 20)
...
```{r}
rich_psR <- estimate_richness(psR, measures = c("Chao1","Shannon", "InvSimpson"))
rich <- list(rich_psR, sample_data(psR)$SampleID, sample_data(psR)$Location, sample_data(psR)$Supplemental_feeding, sample_data(psR)$Hatch_Site, sample_data(psR)$Sex, sample_data(psR)$Habitat)
names(rich) <- c("alpha_diversity", "SampleID", "Location","Supplemental feeding", "Hatch Site", "Sex","Habitat")
```
...
if (shapiro.test(rich$alpha_diversity[["Chao1"]])$p.value > 0.05)
{print(summary(aov(rich$alpha_diversity[["Chao1"]]~rich$`Supplemental feeding`, data = rich)))} else {print(kruskal.test(rich$alpha_diversity$Chao1, rich$`Supplemental feeding`))}
...
# Hatch Site
if (shapiro.test(rich$alpha_diversity[["Chao1"]])$p.value > 0.05)
{print(summary(aov(rich$alpha_diversity[["Chao1"]]~rich$`Hatch Site`, data = rich)))} else {print(kruskal.test(rich$alpha_diversity$Chao1, rich$`Hatch Site`))}

if (shapiro.test(rich$alpha_diversity[["Shannon"]])$p.value > 0.05)
{print(summary(aov(rich$alpha_diversity[["Shannon"]]~rich$`Hatch Site`, data = rich)))} else {print(kruskal.test(rich$alpha_diversity$Shannon, rich$`Hatch Site`))}

if (shapiro.test(rich$alpha_diversity[["InvSimpson"]])$p.value > 0.05)
{print(summary(aov(rich$alpha_diversity[["InvSimpson"]]~rich$`Hatch Site`, data = rich)))} else {print(kruskal.test(rich$alpha_diversity$InvSimpson, rich$`Hatch Site`))}

# Nest Site
if (shapiro.test(rich$alpha_diversity[["Chao1"]])$p.value > 0.05)
{print(summary(aov(rich$alpha_diversity[["Chao1"]]~rich$`Nest Site`, data = rich)))} else {print(kruskal.test(rich$alpha_diversity$Chao1, rich$`Nest Site`))}

if (shapiro.test(rich$alpha_diversity[["Shannon"]])$p.value > 0.05)
{print(summary(aov(rich$alpha_diversity[["Shannon"]]~rich$`Nest Site`, data = rich)))} else {print(kruskal.test(rich$alpha_diversity$Shannon, rich$`Nest Site`))}

if (shapiro.test(rich$alpha_diversity[["InvSimpson"]])$p.value > 0.05)
{print(summary(aov(rich$alpha_diversity[["InvSimpson"]]~rich$`Nest Site`, data = rich)))} else {print(kruskal.test(rich$alpha_diversity$InvSimpson, rich$`Nest Site`))}

# Sex
if (shapiro.test(rich$alpha_diversity[["Chao1"]])$p.value > 0.05)
{print(summary(aov(rich$alpha_diversity[["Chao1"]]~rich$Sex, data = rich)))} else {print(kruskal.test(rich$alpha_diversity$Chao1, rich$Sex))}
...
# Habitat type
if (shapiro.test(rich$alpha_diversity[["Chao1"]])$p.value > 0.05)
{print(t.test(rich$alpha_diversity[["Chao1"]]~rich$Habitat, data = rich))} else {print(wilcox.test(alpha_diversity$Chao1 ~ Habitat, data=rich))}
...
names(rich_df)[names(rich_df) == "alpha_diversity.Chao1"] <- "Chao1"
...
chao1 <- ggplot(rich_df, aes(x=Location, y=Chao1))  
chao1.plot <- chao1 + geom_boxplot(size=1.0, aes(fill=Location)) +   
            myadptheme + theme(plot.title = element_text(size = 35)) +
            scale_fill_manual(values = takahe_pal) +
            labs(y = "Chao1 richness", x = "Location") +
            ggtitle("ANOVA p = 0.05")
chao1.plot 
...
library(ggpubr)
location.adp <- ggarrange(chao1.plot, shannon.plot, simpson.plot, labels = c("A"), font.label = list(size = 30),
...
chao1SF <- ggplot(rich_df, aes(x=Supplemental.feeding, y=Chao1))  
chao1SF.plot <- chao1SF + geom_boxplot(size=1.0, aes(fill=Supplemental.feeding)) +   
            myadptheme + theme(plot.title = element_text(size = 35)) +
            scale_fill_manual(values = f_eight, name = "Supplemental feeding") +
            labs(y = "Chao1 richness", x = "Supplemental feeding") +
            ggtitle("ANOVA p = 0.6")
chao1SF.plot
...
```{r fig.height=10, fig.width=25}
SF.adp <- ggarrange(chao1SF.plot, shannonSF.plot, simpsonSF.plot, labels = c("B"), font.label = list(size = 30),
          ncol=3, nrow=1, common.legend = T , legend = "bottom")
...
chao1.H <- ggplot(rich_df, aes(x=Habitat, y=Chao1))  
chao1.H.plot <- chao1.H + geom_boxplot(size=1.0, aes(fill=Habitat)) +   
            myadptheme + theme(plot.title = element_text(size = 35)) +
            scale_fill_manual(values = c("#5A9599BF","#84D7E1BF")) +
            labs(y = "Chao1 richness", x = "Habitat") +
            ggtitle("ANOVA p = 0.002**")
...
```{r fig.height=10, fig.width=25}
Habitat.adp <- ggarrange(chao1.H.plot, shannon.H.plot, simpson.H.plot, labels = c("C"), font.label = list(size = 30),
          ncol=3, nrow=1, common.legend = T , legend = "bottom")
